...
--- Find all multiline comments in a Lua code string
---@param content string The Lua code string to analyze
---@return table<number, boolean> line_status Table mapping line numbers to comment status (true = comment)
function M.find_multiline_comments(content)
  ...
  for i, line_text in ipairs(lines) do
    ...
    if logger and logger.debug then
      logger.debug("Comment detection", {
        line_num = i,
        content = line_text:sub(1, 30),
        is_comment = context.line_status[i],
        in_ml_comment = context.in_comment,
        stack_depth = #context.state_stack
      })
    end
  end
  ...
  if context.in_comment then
    logger.debug("Warning: Unclosed multiline comment at end of file")
  end
  
  -- Extra loop to fix common issues
  for i = 1, #lines do
    local line_text = lines[i]
    
    -- Special case: line containing known code patterns is definitely executable code
    if line_text and (
       line_text:match("^%s*local%s+") or 
       line_text:match("^%s*function%s+") or
       line_text:match("^%s*if%s+") or
       line_text:match("^%s*for%s+") or
       line_text:match("^%s*while%s+") or
       line_text:match("^%s*return%s+") or
       line_text:match("%w+%s*=%s*%w+") or  -- Variable assignment 
       line_text:match("%w+%s*=%s*[\"']") or -- String assignment
       line_text:match("%w+%s*=%s*%d+") or  -- Number assignment
       line_text:match("%w+%s*%+%s*%d+") or -- Arithmetic operation
       line_text:match("%(%s*%w+%s*%)") or  -- Function call
       line_text:match("^%s*%w+%.%w+")) then -- Table access
      context.line_status[i] = false
    end
  end
...
--- Process a line of code to track multiline comment state
---@param line_text string The line text to process
---@param line_num number The line number being processed
---@param context table The comment tracking context (created with create_multiline_comment_context)
---@return boolean is_comment_line Whether this line is a comment line
function M.process_line_for_comments(line_text, line_num, context)
  ...
  -- If we're already in a multiline comment, check for end
  if context.in_comment then
    -- Find the end of the multiline comment
    local level = context.state_stack[#context.state_stack] or ""
    local pattern = "%]" .. string.rep("=", #level) .. "%]"
    local end_pos = line_text:find(pattern)
    ...
    if end_pos then
      ...
      local after_comment = line_text:sub(end_pos + 2 + #level)
      if after_comment:match("^%s*$") or after_comment:match("^%s*%-%-") then
        -- Nothing but whitespace or another comment after
        context.line_status[line_num] = true
        return true
      else
        -- There's executable code after the comment end
  ...
  -- Check for single-line comments at the start of the line
  local single_comment_pos = line_text:find("%-%-[^%[]")
  if single_comment_pos and (single_comment_pos == 1 or not line_text:find("%S", 1, single_comment_pos-1)) then
  ...
  if ml_comment_pos then
    -- Find the opening bracket level
    local _, _, equals = line_text:find("%-%-(%[=*)%[", ml_comment_pos)
    equals = equals or ""
    
    -- Check if there's code before the comment
    local before_comment = line_text:sub(1, ml_comment_pos - 1)
    if not before_comment:match("^%s*$") then
      -- There's code before the comment
      context.line_status[line_num] = false
      
      -- Check if the comment also ends on this line
      local end_pattern = "%]" .. equals .. "%]"
      local end_pos = line_text:find(end_pattern, ml_comment_pos + 2 + #equals)
      
      if not end_pos then
        -- Multiline comment continues to next line
        context.in_comment = true
        table.insert(context.state_stack, equals)
      end
      
      return false
    end
    
    -- No code before comment, check if it ends on this line
    local end_pattern = "%]" .. equals .. "%]"
    local end_pos = line_text:find(end_pattern, ml_comment_pos + 2 + #equals)
    
    if end_pos then
      -- Comment both starts and ends on this line
      local after_comment = line_text:sub(end_pos + 2 + #equals)
      if after_comment:match("^%s*$") or after_comment:match("^%s*%-%-") then
        -- No code after the comment
        context.line_status[line_num] = true
        return true
      else
  ...
  -- Check for single line comments in the middle of a line
...
--- Classify a single line of Lua code without context from surrounding lines
---@param file_path string Path to the Lua file
---@param line_num number Line number to classify
---@return string line_type The classification of the line (from M.LINE_TYPES)
function M.classify_line_simple(file_path, line_num)
  ...
  if not content then
    logger.warn("Failed to read file for line classification", {
      file_path = file_path,
  ...
  -- Split into lines more reliably (handles last line with no newline)
  local lines = {}
  for line in (content .. "\n"):gmatch("([^\r\n]*)[\r\n]") do
    table.insert(lines, line)
  end
  ...
  -- Line might be out of range
  if line_num > #lines then
    logger.debug("Line number out of range", {
      file_path = file_path,
      line_num = line_num,
  ...
  local context = M.create_multiline_comment_context()
  for i = 1, line_num do
    -- Important: process all lines with the same context to maintain state
  ...
  if context.line_status[line_num] then
    logger.debug("Line classified as comment (non-executable)", {
      file_path = file_path,
      line_num = line_num,
      line_text = line_text:sub(1, 40) -- Truncate for logging
    })
    return M.LINE_TYPES.NON_EXECUTABLE
  end
  ...
  elseif line_text:match("^%s*end%s*$") or
    ...
    if config and config.control_flow_keywords_executable then
      return M.LINE_TYPES.EXECUTABLE
    else
      return M.LINE_TYPES.END_BLOCK
    end
...
-- Enhanced function to process If blocks with condition tracking and proper nesting
local function process_if_block(blocks, parent_block, node, content, block_id_counter, parent_id)
  ...
  -- Initialize parent_block.conditions if it doesn't exist
  parent_block.conditions = parent_block.conditions or {}
  ...
  if node[1] then
    ...
    -- Create a specific block for the condition itself
    block_id_counter = block_id_counter + 1
    condition_id = "cond_" .. block_id_counter
    
    -- Try to get precise line information for the condition
    local cond_start, cond_end
    if node[1].pos and node[1].end_pos then
      cond_start = get_line_for_position(content, node[1].pos)
      cond_end = get_line_for_position(content, node[1].end_pos)
    else
      -- Fallback to approximation based on the if statement position
      cond_start = get_line_for_position(content, node.pos)
      cond_end = cond_start -- Condition is typically on one line
    end
    
    if cond_start and cond_end and cond_start <= cond_end then
      local condition_block = {
        ...
        metadata = {
          ...
          ast_pos = node[1].pos,
          ast_end_pos = node[1].end_pos
        }
      }
      
      table.insert(blocks, condition_block)
      table.insert(parent_block.conditions, condition_id)
    end
  end
  
  -- Store the parent block start and end line for proper block boundaries
  local if_start_line = get_line_for_position(content, node.pos)
  local if_end_line = get_line_for_position(content, node.end_pos)
  
  if if_start_line and if_end_line then
    parent_block.start_line = if_start_line
    parent_block.end_line = if_end_line
  end
  
  -- Process then branch with enhanced boundary detection
  if node[2] and node[2].pos and node[2].end_pos then
    block_id_counter = block_id_counter + 1
    then_id = "then_" .. block_id_counter
    
    -- Get proper then block boundaries
    local then_start = get_line_for_position(content, node[2].pos)
    local then_end = get_line_for_position(content, node[2].end_pos)
    
    -- Attempt to be more precise about the then block start
    -- For simple if statements, the true start is after the 'then' keyword
    if content and node.pos then
      local then_keyword_pos = content:find("%sthen%s", node.pos)
      if then_keyword_pos then
        local then_after_keyword = get_line_for_position(content, then_keyword_pos + 5)
        if then_after_keyword and then_after_keyword <= then_start then
          then_start = then_after_keyword
        end
      end
    end
    ...
    if then_start and then_end and then_start <= then_end then
      local then_block = {
        ...
        metadata = {
          is_then_branch = true,
          ast_pos = node[2].pos,
          ast_end_pos = node[2].end_pos,
          condition_id = condition_id
        }
      }
      
      table.insert(blocks, then_block)
      table.insert(parent_block.branches, then_id)
    end
  end
  
  -- Process else branch with enhanced handling for elseif chains
  if node[3] and node[3].pos and node[3].end_pos then
    block_id_counter = block_id_counter + 1
    else_id = "else_" .. block_id_counter
    
    -- Get proper else block boundaries
    local else_start = get_line_for_position(content, node[3].pos)
    local else_end = get_line_for_position(content, node[3].end_pos)
    
    -- Attempt to be more precise about the else block start
    -- For simple else statements, the true start is after the 'else' keyword
    if content and node[2] and node[2].end_pos then
      local else_keyword_pos = content:find("%selse%s", node[2].end_pos)
      if else_keyword_pos then
        local else_after_keyword = get_line_for_position(content, else_keyword_pos + 5)
        if else_after_keyword and else_after_keyword <= else_start then
          else_start = else_after_keyword
        end
      end
    end
    ...
    if else_start and else_end and else_start <= else_end then
      ...
      local else_block = {
        id = else_id,
        type = is_elseif and "elseif_block" or "else_block",
        ...
        metadata = {
          ...
          is_elseif = is_elseif,
          ast_pos = node[3].pos,
          ast_end_pos = node[3].end_pos,
          condition_id = condition_id
        }
      }
      
      table.insert(blocks, else_block)
      table.insert(parent_block.branches, else_id)
      
      -- Recurse into elseif to properly handle elseif chains
      if is_elseif then
        -- Process this as another if block, with the else_block as parent
        block_id_counter = process_if_block(blocks, else_block, node[3], content, block_id_counter, else_id)
      end
    end
  end
  
  -- Debug logging to help understand block boundaries
  if logger.is_debug_enabled() then
    logger.debug("If block processed", {
      parent_id = parent_id,
      if_start_line = if_start_line,
      if_end_line = if_end_line,
      condition_id = condition_id,
      then_id = then_id,
      else_id = else_id,
      has_elseif = node[3] and node[3].tag == "If"
    })
  end
...
local function find_blocks(ast, blocks, content, parent_id)
    ...
    if block.parent_id ~= "root" then
      ...
      if parent_block then
        ...
        if block.start_line and block.end_line then
          ...
          if parent_block.start_line > block.start_line then
            parent_block.start_line = block.start_line
          end
          
          if parent_block.end_line < block.end_line then
            parent_block.end_line = block.end_line
          end
        end
        ...
        if logger.is_debug_enabled() then
          logger.debug("Deferred parent-child relationship", {
            ...
            reason = "parent block not found in first pass"
          })
        end
      end
    end
    ...
    if parent_block then
      ...
      if logger.is_debug_enabled() then
        logger.warn("Unresolved parent-child relationships", {
          parent_id = parent_id,
          pending_children = #child_ids,
          reason = "parent block not found after processing"
        })
      end
  ...
  for _, block in ipairs(blocks) do
    -- Ensure depth is set
    if not block.depth then
      block.depth = block.parent_id == "root" and 1 or 0
    end
...
-- Get blocks containing a specific line with enhanced nested block tracking
function M.get_blocks_for_line(code_map, line_num)
  ...
  -- First create a block map for efficient lookup
  for _, block in ipairs(code_map.blocks) do
    block_map[block.id] = block
  end
  
  -- Find all blocks that contain this line directly
  for _, block in ipairs(code_map.blocks) do
  ...
  -- Sort blocks by nesting depth for predictable processing
  ...
  table.sort(blocks, function(a, b)
    ...
    -- Secondary sort by start line (earlier blocks first)
  ...
  if logger.is_debug_enabled() then
    logger.debug("Blocks for line " .. line_num, {
      line_num = line_num,
      block_count = #blocks,
    ...
    for i, block in ipairs(blocks) do
      logger.debug("Block for line details", {
        index = i,
        id = block.id,
        type = block.type,
