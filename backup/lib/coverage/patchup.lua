...
-- Access global coverage data from debug_hook module (for proper coverage tracking)
...
local function is_in_multiline_comment(line, file_path, line_num, file_data)
  ...
  if not file_path or not line_num then
    -- Improved pattern matching for multiline comments
    -- Handle --[[ and --[=[ style comments
    local starts = line and line:match("^%s*%-%-%[=?%[")
    local ends = line and line:match("%]=?%]")
  ...
  if file_data and file_data.source then
    for i = 1, line_num do
      ...
      -- Improved pattern matching for multiline comments
      -- Handle --[[ and --[=[ style comments
      local starts = current_line:match("^%s*%-%-%[=?%[")
      local ends = current_line:match("%]=?%]")
      ...
      for i = 1, file_data.line_count do
        ...
        if type(line_info) == "table" then
          if not line_info.executable then
            ...
            -- General solution: For all files, mark as covered if tracked and executed
            -- This ensures consistent coverage status across all files
            
            -- Check both _executed_lines table and track_all_executed config setting
            local track_all_executed = false
            
            -- Use central_config to get the track_all_executed setting
            local central_config = require("lib.core.central_config")
            if central_config then
              track_all_executed = central_config.get("coverage.track_all_executed", false)
            end
            
            -- Check execution data from all possible tracking sources
            local was_executed = false
            
            -- 1. Check local file execution tracking
            if file_data._executed_lines and file_data._executed_lines[i] then
              was_executed = true
              logger.debug("Line execution detected via _executed_lines", {
                line = i,
                operation = "patch_file.check_execution"
              })
            end
            
            -- 2. Check global execution tracking (used by some code paths)
            local line_key = file_path .. ":" .. i
            if not was_executed and coverage_data.executed_lines and coverage_data.executed_lines[line_key] then
              was_executed = true
              logger.debug("Line execution detected via global executed_lines", {
                line = i,
                operation = "patch_file.check_execution"
              })
            end
            
            -- 3. Check execution counts
            if not was_executed and file_data._execution_counts and file_data._execution_counts[i] and file_data._execution_counts[i] > 0 then
              was_executed = true
              logger.debug("Line execution detected via _execution_counts", {
                line = i,
                count = file_data._execution_counts[i],
                operation = "patch_file.check_execution"
              })
            end
            
            -- 4. Check file's lines table for execution evidence
            if not was_executed and file_data.lines and file_data.lines[i] and 
               type(file_data.lines[i]) == "table" and file_data.lines[i].executed then
              was_executed = true
              logger.debug("Line execution detected via lines table", {
                line = i,
                operation = "patch_file.check_execution"
              })
            end
            
            -- Mark as executed based on evidence or configuration
            if was_executed or track_all_executed then
              -- Ensure the line is properly marked as covered in the lines table
              if not file_data.lines[i] then 
                file_data.lines[i] = {}
              end

              -- Ensure all line properties are explicitly set to avoid nil values
              file_data.lines[i].executable = true
              file_data.lines[i].executed = true
              file_data.lines[i].covered = true
              
              -- Record the line as executed in file-specific execution tracking
              if not file_data._executed_lines then
                file_data._executed_lines = {}
              end
              file_data._executed_lines[i] = true
              
              -- Ensure execution counts are recorded
              if not file_data._execution_counts then
                file_data._execution_counts = {}
              end
              file_data._execution_counts[i] = file_data._execution_counts[i] or 1
              
              -- Ensure it's also tracked in global tables for consistency across all tracking mechanisms
              if coverage_data then
                -- Track in executed_lines global table
                if not coverage_data.executed_lines then
                  coverage_data.executed_lines = {}
                end
                coverage_data.executed_lines[line_key] = true
                
                -- Track in covered_lines global table
                if not coverage_data.covered_lines then
                  coverage_data.covered_lines = {}
                end
                coverage_data.covered_lines[line_key] = true
              end
              
              -- CRITICAL: Ensure executable_lines status is set in both locations
              file_data.executable_lines[i] = true
            end
    ...
    for file_path, file_data in pairs(coverage_data.files) do
      ...
      if file_path:match("^/tmp/lua_") and not fs.file_exists(file_path) then
        logger.debug("Skipping non-existent temporary file", {
          file_path = file_path,
          operation = "patch_all.filter_temp_files"
        })
        skipped_files = skipped_files + 1
        goto continue
      end
      
      -- Using a general mechanism for all files with executed lines
      ...
      -- Check for execution data in multiple tracking data structures
      local executed_lines = {}
      
      -- Source 1: Direct _executed_lines table in file data
      ...
      -- Source 2: Execution counts table, if available
      ...
      -- Source 3: Global executed lines table for cross-file consistency
      for line_key, _ in pairs(coverage_data.executed_lines or {}) do
        local line_file, line_num = line_key:match("(.+):(%d+)")
        if line_file == file_path then
      ...
      -- Now mark all executed lines as covered
      local line_count = 0
      for line_num, _ in pairs(executed_lines) do
        if not file_data.executable_lines or file_data.executable_lines[line_num] ~= false then
          -- Mark any executed line as both executed and covered
          ...
          -- Also ensure the executable_lines table is updated
          if file_data.executable_lines then
            file_data.executable_lines[line_num] = true
          end
          
          -- Mark as covered in the global tracking table
      ...
      -- Mark file as active to ensure it appears in reports
      file_data.active = true
      file_data.discovered = true
      
      logger.debug("Processed executed lines during patching", {
        file_path = file_path,
