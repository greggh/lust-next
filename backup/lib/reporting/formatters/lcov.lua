...
local function process_lines(lcov_lines, file_data, config)
  ...
  for line_num, is_covered in pairs(file_data.lines) do
    ...
    local line_output_success, line_output = error_handler.try(function()
      ...
      if config.use_actual_execution_counts and 
         file_data._execution_counts and 
         file_data._execution_counts[line_num] and
         file_data._execution_counts[line_num] > 0 then
        -- Use actual execution count from debug hook tracking
        output = output .. tostring(file_data._execution_counts[line_num])
        -- Also mark as covered for consistency
        is_covered = true
      else
...
local function process_file(lcov_lines, filename, file_data, config)
  ...
  if file_data.total_functions and file_data.total_functions > 0 then
    add_record(lcov_lines, "FNF:" .. file_data.total_functions)
    add_record(lcov_lines, "FNH:" .. (file_data.covered_functions or 0))
  end
