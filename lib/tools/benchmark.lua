...
local function format_time(time_seconds)
  -- Validate input
  error_handler.assert(type(time_seconds) == "number", 
...
local function calculate_stats(measurements)
  -- Validate input
  error_handler.assert(type(measurements) == "table", 
    "measurements must be a table", 
    error_handler.CATEGORY.VALIDATION,
    {measurements_type = type(measurements)}
  )
  
  error_handler.assert(#measurements > 0, 
  ...
  local success, stats, err = error_handler.try(function()
    ...
    for idx, time in ipairs(measurements) do
      error_handler.assert(type(time) == "number", 
...
function benchmark.measure(func, args, options)
  ...
  error_handler.assert(type(func) == "function", 
    "benchmark.measure requires a function to benchmark", 
    error_handler.CATEGORY.VALIDATION,
    {func_type = type(func)}
  )
  ...
  for i = 1, warmup do
    if gc_before then 
      error_handler.try(function() collectgarbage("collect") end)
    end
    
    -- Measure warmup execution
    local start_time = high_res_time()
    local start_memory = error_handler.try(function() return collectgarbage("count") end) or 0
    ...
    local end_memory = error_handler.try(function() return collectgarbage("count") end) or start_memory
  ...
  results.memory_stats = memory_stats_success and memory_stats or {
    mean = 0, min = 0, max = 0, std_dev = 0, count = #results.memory, total = 0
  }
...
function benchmark.suite(suite_def, options)
  ...
  local success, config = error_handler.try(function()
    ...
    error_handler.assert(type(benchmarks) == "table", 
  ...
  for idx, benchmark_def in ipairs(config.benchmarks) do
    local bench_success, bench_result = error_handler.try(function()
      ...
      error_handler.assert(benchmark_def.func ~= nil, 
        "Benchmark function is required", 
        error_handler.CATEGORY.VALIDATION,
        {benchmark_name = name}
      )
      
      error_handler.assert(type(benchmark_def.func) == "function", 
...
function benchmark.compare(benchmark1, benchmark2, options)
  ...
  local compare_success, comparison = error_handler.try(function()
    ...
    error_handler.assert(config.benchmark2.time_stats.mean ~= 0, 
      "benchmark2.time_stats.mean cannot be zero", 
      error_handler.CATEGORY.VALIDATION,
      {mean = config.benchmark2.time_stats.mean}
    )
    
    error_handler.assert(config.benchmark2.memory_stats.mean ~= 0, 
...
function benchmark.print_result(result, options)
  -- Validate required parameters
  error_handler.assert(result ~= nil, 
  ...
  local success, config = error_handler.try(function()
    ...
    error_handler.assert(type(result) == "table", 
      "result must be a table", 
      error_handler.CATEGORY.VALIDATION,
      {result_type = type(result)}
    )
    
    error_handler.assert(type(result.time_stats) == "table", 
      "result.time_stats must be a table", 
      error_handler.CATEGORY.VALIDATION,
      {has_time_stats = type(result.time_stats) == "table"}
    )
    
    error_handler.assert(type(result.memory_stats) == "table", 
  ...
  error_handler.safe_io_operation(function()
    ...
    if config.report_stats then
      ...
      io.write(string.format("  Std Dev: %s (%.1f%%)\n", 
        format_time(config.result.time_stats.std_dev),
        percent
      ))
    end
...
function benchmark.generate_large_test_suite(options)
  ...
  local function generate_tests(level, prefix)
    return error_handler.try(function()
      ...
      local tests_at_level = level == config.nesting_level and config.tests_per_file or math.ceil(config.tests_per_file / level)
      local test_content = ""
  ...
  for i = 1, config.file_count do
    ...
    local content_success, content = error_handler.try(function()
      local header = "-- Generated large test suite file #" .. i .. "\n" ..
  ...
  if not config.silent then
    error_handler.safe_io_operation(function()
      io.write("Generated " .. success_count .. " test files with approximately " .. 
            (success_count * config.tests_per_file) .. " total tests in " .. config.output_dir .. "\n")
...
function benchmark.register_with_lust(lust_next)
  -- Validate input
  error_handler.assert(lust_next ~= nil, 
    "lust_next must be provided", 
    error_handler.CATEGORY.VALIDATION,
    {lust_next_provided = lust_next ~= nil}
  )
  
  error_handler.assert(type(lust_next) == "table", 
    "lust_next must be a table", 
    error_handler.CATEGORY.VALIDATION,
    {lust_next_type = type(lust_next)}
  )
  ...
  if not success then
    logger.error("Failed to register benchmark module with lust-next")
    return lust_next
  end
  
  logger.debug("Benchmark module registered with lust-next")
  return lust_next
end
