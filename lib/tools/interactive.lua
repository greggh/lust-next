    ...
    if interactive_config then
      ...
      for key, default_value in pairs(DEFAULT_CONFIG) do
        ...
        if key ~= "watch_dirs" and key ~= "exclude_patterns" then
          state[key] = interactive_config[key] ~= nil
                     and interactive_config[key]
                     or default_value
        end
      end
...
local function print_help()
  ...
  logger.debug("Help information displayed", {
    ...
    available_commands = {
      "help", "run", "list", "filter", "focus", "tags", "watch", "watch-dir",
      "watch-exclude", "codefix", "dir", "pattern", "clear", "status", "history", "exit"
    }
  })
end
