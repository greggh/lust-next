...
function spy.new(fn)
  ...
  if not mt_success then
    local error_obj = error_handler.runtime_error(
      "Failed to set up spy metatable",
      {
        function_name = "spy.new",
        fn_type = type(fn)
      },
  ...
  local function make_method_callable_prop(obj, method_name, method_fn)
    ...
    if obj == nil then
      local err = error_handler.validation_error(
        "Cannot add method to nil object",
        {
    ...
    if type(method_fn) ~= "function" then
      local err = error_handler.validation_error(
        "Method function must be a function",
        {
          function_name = "make_method_callable_prop",
  ...
  function spy_obj:called_before(other_spy, call_index)
    ...
    if type(other_spy) ~= "table" then
      local err = error_handler.validation_error(
        "called_before requires a spy object as argument",
        {
          function_name = "spy_obj:called_before",
          parameter_name = "other_spy",
    ...
    if not other_spy.call_sequence then
      local err = error_handler.validation_error(
        "called_before requires a spy object with call_sequence",
        {
          function_name = "spy_obj:called_before",
          parameter_name = "other_spy",
          is_spy = other_spy._is_lust_spy or false
        }
      )
      logger.error(err.message, err.context)
      return false
  ...
  function spy_obj:called_after(other_spy, call_index)
    ...
    if type(other_spy) ~= "table" then
      local err = error_handler.validation_error(
        "called_after requires a spy object as argument",
        {
          function_name = "spy_obj:called_after",
          parameter_name = "other_spy",
          provided_type = type(other_spy)
    ...
    if not other_spy.call_sequence then
      local err = error_handler.validation_error(
        "called_after requires a spy object with call_sequence",
        {
          function_name = "spy_obj:called_after",
          parameter_name = "other_spy",
          is_spy = other_spy._is_lust_spy or false
  ...
  local final_check_success, _ = error_handler.try(function()
    if not spy_obj.calls then spy_obj.calls = {} end
    if not spy_obj.call_sequence then spy_obj.call_sequence = {} end
    if not spy_obj.call_history then spy_obj.call_history = {} end
    return true
  end)
...
function spy.on(obj, method_name)
  ...
  if type(obj) ~= "table" then
    local err = error_handler.validation_error(
      "spy.on requires a table as its first argument",
      {
        function_name = "spy.on",
        parameter_name = "obj",
        expected = "table",
  ...
  if type(obj[method_name]) ~= "function" then
    local err = error_handler.validation_error(
      "Method exists but is not a function",
      {
        function_name = "spy.on",
        parameter_name = "method_name",
        method_name = method_name,
  ...
  function spy_obj:restore()
    ...
    if self.target == nil or self.name == nil then
      local err = error_handler.validation_error(
        "Cannot restore spy with nil target or name",
        {
  ...
  if not success then
    ...
    local error_obj = error_handler.runtime_error(
      "Failed to replace method with spy",
      {
        function_name = "spy.on",
        method_name = method_name,
        target_type = type(obj)
